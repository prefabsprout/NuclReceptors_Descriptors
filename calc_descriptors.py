...
def ch_clamp_angles(pdb_file, charge_clamps):
    ...
    for elem in range(len(clamp_vectors)):
        angles[f'{charge_clamps[elem]}-{charge_clamps[elem - 1]}-{charge_clamps[elem - 2]}'] = \
            degrees(calc_angle(clamp_vectors[charge_clamps[elem]], clamp_vectors[charge_clamps[elem - 1]],
...
if __name__ == '__main__':
    def calc_all(directory, db_output, clamp_resid, species, prep):
        ...
        for filename in os.listdir(dir):
            ...
            df_concat = df.merge(df_sse, on='prot_name').merge(df_len, on='prot_name') \
